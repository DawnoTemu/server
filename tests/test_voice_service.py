...
class TestVoiceService:
    ...
    def test_clone_voice_elevenlabs(self, mock_elevenlabs_session, sample_audio_file):
        ...
        with patch('config.Config.PREFERRED_VOICE_SERVICE', 'elevenlabs'):
            with patch('config.Config.ELEVENLABS_API_KEY', 'test_key'):
                with patch('utils.audio_splitter.split_audio_file', return_value=[("chunk.mp3", sample_audio_file, "audio/mpeg")]):
    ...
    def test_clone_voice_cartesia(self, sample_audio_file):
        """Test cloning voice with Cartesia"""
        mock_voice = MagicMock()
        mock_voice.id = "test-voice-id-789"
        mock_voice.name = "Test Voice"

        with patch('config.Config.PREFERRED_VOICE_SERVICE', 'cartesia'):
            with patch('config.Config.CARTESIA_API_KEY', 'test_key'):
                with patch('utils.cartesia_sdk_service.CartesiaSDKService.clone_voice',
                           return_value=(True, {"voice_id": "test-voice-id-789", "name": "Test Voice"})):
                    success, data = VoiceService.clone_voice(
                        file_data=sample_audio_file,
                        filename="test_voice.mp3",
                        user_id=123,
    ...
    def test_delete_voice_cartesia(self):
        """Test deleting voice with Cartesia"""
        with patch('config.Config.PREFERRED_VOICE_SERVICE', 'cartesia'):
            with patch('config.Config.CARTESIA_API_KEY', 'test_key'):
                with patch('utils.cartesia_sdk_service.CartesiaSDKService.delete_voice',
                           return_value=(True, "Voice deleted")):
                    success, message = VoiceService.delete_voice(
                        voice_id=123,
    ...
    def test_synthesize_speech_cartesia(self):
        """Test synthesizing speech with Cartesia"""
        mock_audio = BytesIO(b'mock cartesia audio')
        with patch('config.Config.PREFERRED_VOICE_SERVICE', 'cartesia'):
            with patch('config.Config.CARTESIA_API_KEY', 'test_key'):
                with patch('utils.cartesia_sdk_service.CartesiaSDKService.synthesize_speech',
                           return_value=(True, mock_audio)):
                    success, audio_data = VoiceService.synthesize_speech(
                        external_voice_id="test-voice-id-789",
    ...
    def test_fallback_when_service_not_available(self, mock_elevenlabs_session, sample_audio_file):
        """Test falling back to another service when the preferred one is not available"""
        with patch('config.Config.PREFERRED_VOICE_SERVICE', 'elevenlabs'):
            with patch('config.Config.ELEVENLABS_API_KEY', None):
                with patch('config.Config.CARTESIA_API_KEY', 'test_key'):
                    with patch('utils.cartesia_sdk_service.CartesiaSDKService.clone_voice',
                               return_value=(True, {"voice_id": "fallback-id", "name": "Fallback"})):
                        success, data = VoiceService.clone_voice(
                            file_data=sample_audio_file,
                            filename="test_voice.mp3",
                            user_id=123,
